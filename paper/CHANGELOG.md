...
- Seeded `src/main.tex` and `bibliography/references.bib` byte-identical from that baseline. They
  are working sources from this point on. The imported baseline itself is not tracked in this
  repository; it is identified by the digests above, and nothing here is claimed to be it.
...
`CHANGELOG-WM-1.0-to-1.1.md` beside this file, which is an imported historical document and is not
rewritten here.
...
## A note on one stale path inside the source

`src/main.tex` line 25 carries a comment naming `paper/witnessability-model/metadata.yaml`. That
directory no longer exists — the source moved up to `paper/` — and the comment is wrong.

It stays wrong on purpose. `main.tex` is the source of the released WM 1.1 artifact, and its bytes
are what `make reproduce-release` proves; editing a comment would change the file whose digest
`84d7a88c…f48e94` the release record pins. The right file is `metadata.yaml` beside it, and the
comment will be corrected in the next revision, where the source is allowed to change.
